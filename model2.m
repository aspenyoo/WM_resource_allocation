...
% parameters
sigmaVec = [5 7 9];
Jbar = 1.8;     % mean parameter of gamma distribution
tauVec = [1 0.001];%0.008;        % scale parameter of gamma distribution
beta = 1;%15;
...
%     blah1 = gaminv([.005 .995]);
%     
%     ind1 = []; ind2 = [];
%     blah1 = 0.001;
%     blah2 = 20;
%     while isempty(ind2) || isempty(ind2); % make range smaller from right side
%         blah2 = blah2 * 0.9;
%         xx = linspace(blah1,blah2,nsamp);
%         yy = gampdf(xx,Jbar/tau,tau);
%         plot(xx,yy); pause(0.2)
...
%     plot(D(icond,:),error_sacc(icond,:),'o','Color',colors(icond,:),'MarkerSize',2);hold on;
    %     blah = [0 10];
...
% plot data 
figure;
plot(D(1,:),error_sacc(1,:),'o','Color',colors(1,:),'MarkerSize',2);
defaultplot
